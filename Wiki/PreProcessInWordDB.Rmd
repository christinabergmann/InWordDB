---
title: "InWordDB: Reading in the database and preprocessing it"
...
The most recent database is always provided in the Files section under the name _InWordDB.csv_.
...
library(RCurl)
linkdb = "https://raw.githubusercontent.com/christinabergmann/InWordDB/master/InWordDB.csv"
db <- getURL(linkdb)
...
#How many native languages are represented in the database
library(knitr)
kable(table(db$NativeLanguage), format = "html", col.names = c("Native Language", "Number"), padding = 0.5)
```
...
We need a random seed for reproducibility. Now _impute_ from the library _Hmisc_ will always yield the same result, unless the seed is changed.
```{r}
set.seed(111)
